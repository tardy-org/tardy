...
pub fn netListenIpWindows(
    ...
    if (options.reuse_address) try setSocketOptionAfd(socket, ws2_32.SOL.SOCKET, ws2_32.SO.REUSEADDR, true);
...
pub fn netConnectIpWindows(
    ...
    try setSocketOptionAfd(socket, ws2_32.SOL.SOCKET, ws2_32.SO.REUSE_UNICASTPORT, true);

    const wildcard_addr: Socket.Address = switch (address) {
        .in => .{ .in = .{ .port = 0, .addr = 0 } },
        .in6 => .{ .in6 = .{ .port = 0, .addr = @splat(0) } },
        // unix not supported now
        else => unreachable,
    };

    const bound_address = bindSocketIpAfd(
        socket,
        wildcard_addr,
        .Active,
    ...
    const addr_len = blk: switch (address) {
        .in => |in| {
            storage.Socket.in = in;
            break :blk @sizeOf(ws2_32.sockaddr.in);
        },
        .in6 => |in6| {
            storage.Socket.in = in6;
...
pub fn netAcceptWindows(listen_handle: net.Socket.Handle, options: net.Server.AcceptOptions) net.Server.AcceptError!net.Socket {
    const Storage = extern struct {
    ...
    const accept_handle = openSocketAfd(
        storage.RemoteSocket.posix.any.family,
    ...
    return .{
        ...
        .address = storage.RemoteSocket.toAddress(),
    };
}
...
pub fn bindSocketIpAfd(
    ...
    const sock_len = blk: switch (sock_addr) {
        .in => {
            storage.Socket.in = sock_addr.in;
            break :blk @sizeOf(ws2_32.sockaddr.in);
        },
        .in => {
            storage.Socket.in6 = sock_addr.in6;
            break :blk @sizeOf(ws2_32.sockaddr.in6);
        },
    ...
    const bind_in_opt = @as([]const u8, @ptrCast(&storage))[0 .. @offsetOf(Storage, "Address") + sock_len];
